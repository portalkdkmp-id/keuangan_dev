...
export default function Create({ disbursement, remainingAmount }: any) {
    ...
    const accounts =
        form.data.recipient_type === 'pic_kdkmp'
            ? (submission.submitter?.bank_accounts ?? [])
            : form.data.recipient_type === 'cooperative'
    ...
    const submit = () => {
        form.transform((data) => ({
            ...
            transferred_at:
                data.transferred_at.replace('T', ' ') +
                (data.transferred_at.length === 16 ? ':00' : ''),
        }));
        form.post(`/finance/fund-distributions/${disbursement.id}`, {
            forceFormData: true,
        });
    };
    const changeRecipient = (value: string) =>
        form.setData({
            ...
            recipient_user_id:
                value === 'pic_kdkmp' ? submission.submitted_by : '',
            recipient_cooperative_id:
                value === 'cooperative' ? submission.cooperative_id : '',
            destination_bank_account_id: '',
        });

    return (
        <div className="mx-auto max-w-3xl space-y-4 p-4">
            <Head title="Salurkan Dana" />
            <BackButton fallback="/finance/fund-distributions" />
            <div>
                <h1 className="text-2xl font-semibold">Salurkan Dana</h1>
                <p className="text-sm text-muted-foreground">
                    {submission.submission_number} · Dana dari Director{' '}
                    {rupiah(disbursement.amount)} · Sisa{' '}
            ...
            >
                ...
                {form.data.recipient_type !== 'other' ? (
                    <>
                ...
                <Label>Nominal distribusi</Label>
                <MoneyInput
                ...
                <Input
                    ...
                    onChange={(event) =>
                        form.setData('transferred_at', event.target.value)
                    }
                />
                ...
                <MultipleFileInput
                    ...
                    required
                />
                <Button disabled={form.processing}>Simpan Distribusi</Button>
            </form>
        </div>
    );
}
