...
export default function SubmissionsEdit({ submission, cooperatives, requestCategories, requestTypes, bankAccounts }: any) {
    ...
    return <div className="space-y-4 p-4"><Head title="Edit Pengajuan" /><h1 className="text-2xl font-semibold">Edit Pengajuan Dana</h1><SubmissionForm form={form} cooperatives={cooperatives} requestCategories={requestCategories} requestTypes={requestTypes} bankAccounts={bankAccounts} onSubmit={() => form.put(`/submissions/${submission.id}`)} /><SubmissionAttachments submission={submission} editable /></div>;
}
