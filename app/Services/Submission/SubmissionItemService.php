...
class SubmissionItemService
{
    ...
    public function replaceItems(FinancialSubmission $submission, array $items): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            $category = isset($item['category_id']) ? SubmissionCategory::find($item['category_id']) : null;
            $type = isset($item['request_type_id']) ? SubmissionRequestType::findOrFail($item['request_type_id']) : null;
            $subtotal = $this->calculator->itemSubtotal($item['quantity'], $item['unit_price']);
            $rows[] = [
                'financial_submission_id' => $submission->id,
                'category_id' => $category?->id,
                'request_type_id' => $type?->id,
                'category_name' => $category?->name ?? $type?->name ?? 'Lainnya',
