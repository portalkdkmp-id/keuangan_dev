...
class AdvanceService
{
    ...
    public function createDraft(User $actor, array $data): FinancialSubmission
    {
        ...
        return DB::transaction(function () use ($actor, $data) {
            ...
            $submission = FinancialSubmission::create(['submission_number' => $this->numbers->generateAdvanceNumber(), 'type' => SubmissionType::ADVANCE, 'status' => SubmissionStatus::DRAFT, 'is_urgent' => $data['is_urgent'] ?? false, 'submission_request_category_id' => $category->id, 'submission_request_type_id' => $type->id, 'cooperative_id' => $data['cooperative_id'], 'recipient_bank_account_id' => $account->id, 'bank_name_snapshot' => $account->bank_name, 'bank_account_number_snapshot' => $account->account_number, 'bank_account_holder_snapshot' => $account->account_holder_name, 'submitted_by' => $actor->id, 'submitter_city_id' => $actor->city_id, 'title' => $data['title'], 'purpose' => $data['purpose'], 'needed_date' => $data['expected_transaction_date'] ?? null, 'notes' => $data['notes'] ?? null, 'total_amount' => $data['estimated_amount']]);
